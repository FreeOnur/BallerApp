...
4. Local test: `docker compose up -d --build` (repo root) → `http://localhost:8000/docs`.

## Do not

- Expose Postgres publicly in prod (`docker-compose.yml` internal network only; TLS via Coolify).
